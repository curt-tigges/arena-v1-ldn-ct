...
def test_integrate_product(integrate_product):
    ...
    np.testing.assert_allclose(
        integrate_product_true, integrate_product_approx, atol=1e-10
    )
...
def get_title_from_coeffs(a_0, A_n, B_n):
    ...
    return "y = " + f"{0.5*a_0:}" + A_n_coeffs + B_n_coeffs
...
def visualise_fourier_coeff_convergence(x, y, y_pred_list, coeffs_list):
    ...
    fig = go.FigureWidget(
        ...
        layout=go.Layout(
            ...
            showlegend=False,
            yaxis_range=[
                y.min() - 0.5 * (y.max() - y.min()),
                y.max() + 0.5 * (y.max() - y.min()),
            ],
        ),
    )
        ...
        dict(
            ...
            steps=[
                dict(
                    ...
                    args=[
                        {
                            "visible": [True] * len(coeffs_list)
                            + bool_list(i, len(coeffs_list))
                        },
